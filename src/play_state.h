...
void init_play_state(play_state_t *play, renderer_t *renderer, player_t players[2])
{
    *play = (play_state_t)
    {
        .background = renderer_load_texture(renderer, IMAGE_PATH("stage_cava.jpg")),
...
{   
    ...
    const anim_frame_t *col_atk = fighter_get_frame_data(fighter);
    ...
    for_range_i(col_atk->count_hitboxs)
    {
        ...
        hit = to_world_rect(def, col_def->hitboxs[i]);
        renderer_draw_rect(renderer, LAYER_UI1, &hit, COLOR_RED, false);
    }    

    for_range_j(col_atk->count_hurtboxs)
    {
